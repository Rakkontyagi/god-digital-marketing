#!/usr/bin/env python3
"""
Script to fix encoding issues in failed HTML files and re-apply navigation
"""
...
# List of files that failed in the previous run (from manual analysis)
FAILED_FILES = [
    "services/seo-services/belagavi/index.html",
    "services/seo-services/firozabad/index.html",
    "services/seo-services/mathura/index.html",
    "services/seo-services/prayagraj/index.html",
    "services/seo-services/saharanpur/index.html",
    "services/seo-services/siliguri/index.html",
...
def convert_to_utf8(file_path):
    ...
    try:
        ...
        try:
            ...
            # If detected encoding fails, try cp1252 (Windows)
            print(f"  Detected encoding failed, trying cp1252...")
...
def update_html_file(file_path):
    ...
    try:
        # Read the file with UTF-8 (now that it's converted)
        ...
        # Find where to inject navigation
        body_match = re.search(r'</style>\s*</head>\s*<body[^>]*>', content, re.DOTALL)

        if not body_match:
            print(f"  WARNING: Could not find body tag")
            return False

        # Find old nav end
        nav_end = re.search(r'</nav>', content)
        if not nav_end:
            print(f"  INFO: No existing nav, will add new one")
            ...
            if nav_start_match:
                ...
                print(f"  WARNING: Could not find nav start")
                return False
        ...
        if footer_match:
            ...
            if footer_end:
                ...
                print(f"  WARNING: Could not find footer end")
                return False
        else:
            print(f"  INFO: No existing footer, will add new one")
            body_end = re.search(r'</body>', new_content)
            if body_end:
                final_content = new_content[:body_end.start()] + '\n' + new_footer + '\n' + new_content[body_end.start():]
            else:
                print(f"  WARNING: No closing body tag")
                return False
...
def main():
    """Main function"""
    print("==> Fixing encoding issues and updating failed files...")
    print(f"Base directory: {BASE_DIR}\n")

    success_count = 0
    fail_count = 0

    for rel_file in FAILED_FILES:
        ...
        if convert_to_utf8(file_path):
            ...
            if update_html_file(file_path):
                print(f"  [OK] Navigation updated successfully\n")
                success_count += 1
            else:
                print(f"  [FAIL] Navigation update failed\n")
                fail_count += 1
        else:
            fail_count += 1
            print()
    ...
    print("SUMMARY")
    print("=" * 60)
    print(f"Total files attempted: {len(FAILED_FILES)}")
