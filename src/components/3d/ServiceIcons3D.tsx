...
import { Text, Box, Sphere, Torus, Dodecahedron, Cone, Environment } from '@react-three/drei';
...
const FloatingIcon: React.FC<FloatingIconProps> = ({ position, color, shape, serviceName, args = [] }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (textRef.current) {
        // Text from drei handles billboarding by default
    }
  });
  ...
  const geometry = useMemo(() => {
    ...
    switch (shape) {
      case 'sphere':
        return <Sphere args={args.length ? args : [0.5, 32, 32]}><meshStandardMaterial {...materialProps} /></Sphere>;
      case 'torus':
        return <Torus args={args.length ? args : [0.4, 0.15, 16, 100]}><meshStandardMaterial {...materialProps} /></Torus>;
      case 'dodecahedron':
        return <Dodecahedron args={args.length ? args : [0.5]}><meshStandardMaterial {...materialProps} /></Dodecahedron>;
      case 'cone':
        return <Cone args={args.length ? args : [0.4, 0.8, 32]}><meshStandardMaterial {...materialProps} /></Cone>;
      case 'box':
      default:
        return <Box args={args.length ? args : [0.7, 0.7, 0.7]}><meshStandardMaterial {...materialProps} /></Box>;
    }
  ...
  return (
    <div style={{ height: '350px', width: '100%' }}>
      <Canvas camera={{ position: [0, 0.5, 5], fov: 50 }} gl={{ antialias: true, pixelRatio: (typeof window !== 'undefined' ? window.devicePixelRatio : 1) }}>
        ...
        {/* "sunset" preset offers warm tones, complementing the varied icon colors. */}
